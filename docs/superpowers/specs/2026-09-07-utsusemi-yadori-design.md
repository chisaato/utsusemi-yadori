...
- 页面：
  ...
  3. **二进制管理**：三源版本对比、下载/切换激活、上传自编译、删除清理；**列表展示 manifest 全量元数据**（variant/version/arch/来源/sha256/导入时间；具体呈现形式待前端初版后打磨）；
  4. **远程协同**：开关 Gin 服务、展示二维码与 `adb forward` 指引、token 管理；
  5. **日志**：ctl 与 frida-server 日志查看。
- **暗色/亮色主题自适应**：跟随系统 `prefers-color-scheme` 并允许手动切换（Naive UI 原生支持 darkTheme；持久化到 localStorage）。
